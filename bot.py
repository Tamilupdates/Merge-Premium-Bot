...
async def files_handler(c: Client, m: Message):
    ...
    if user.merge_mode == 1:
        ...
        if (
            ...
            markup = await makeButtons(c, m, queueDB)
            await editable.text(
                "Max 30 videos allowed", reply_markup=InlineKeyboardMarkup(markup)
            )
    ...
    if user.merge_mode == 4:
        ...
        if rmess.video or rmess.document:
            ...
            if file_name is None:
                await m.reply("File name not found; goto @Tamilupdates")
                return
    ...
    try:
        with userBot:
            userBot.send_message(
                ...
                text="Merge Bot booted with Premium Account \n\nUse upto 4GB Files",
